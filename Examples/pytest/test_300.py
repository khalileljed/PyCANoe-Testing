...
def speedramp(app, target_speed, accel=5):
    # Gets current speed value and ramps it up/down with accel m/s² . Default accel is 5 m/s².
    curr_speed = app.GetSignalValue("Ethernet", 1, "Charisma_20_XIX_VLAN_Komfort",
                                    "CHA_ReqPrgHsp_XIX_Charisma_20_XIX_VLAN_Komfort")

    moves = ceil((target_speed-curr_speed)/accel)
    for i in range(moves-1):
        curr_speed += accel*1  # time = 1 sec
        app.SetSignalValue("Ethernet", 1, "VDSO_01_XIX_VLAN_Komfort",
                           "VDSO_Vx6d_XIX_VDSO_01_XIX_VLAN_Komfort", curr_speed)
        time.sleep(1)
...
def test_301():
    ...
    app.SetSignalValue(ACTUAL_SPEED['bus_type'], int(ACTUAL_SPEED['channel_num']), ACTUAL_SPEED['msg_name'],
                       ACTUAL_SPEED['sig_name'],  127.7991)
